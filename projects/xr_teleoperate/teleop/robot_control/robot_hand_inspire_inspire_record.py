...
from teleop.robot_control.hand_retargeting_inspire_record import HandRetargeting, HandType
import numpy as np
...
class Inspire_Controller_FTP:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
                       dual_hand_action_array = None, dual_hand_force_array = None, dual_hand_touch_array = None,
        ...
        self.LeftHandTouch_subscriber = ChannelSubscriber("rt/inspire_hand/touch/l", inspire_dds.inspire_hand_touch)
        self.LeftHandTouch_subscriber.Init()
        self.RightHandTouch_subscriber = ChannelSubscriber("rt/inspire_hand/touch/r", inspire_dds.inspire_hand_touch)
        ...
        self.left_hand_touch_array = Array('d', INSPIRE_FTP_TOUCH_NUM_VALUES, lock=True)
        self.right_hand_touch_array = Array('d', INSPIRE_FTP_TOUCH_NUM_VALUES, lock=True)
        ...
        hand_control_process = Process(
            ...
            args=(
                ...
                self.left_hand_force_array,
                self.right_hand_force_array,
                self.left_hand_touch_array,
                self.right_hand_touch_array,
                dual_hand_data_lock,
    ...
    def _subscribe_hand_state(self):
        ...
        while True:
                ...
                if hasattr(left_state_msg, 'angle_act') and len(left_state_msg.angle_act) == Inspire_Num_Motors:
                    ...
                    if hasattr(left_state_msg, 'force_act') and len(left_state_msg.force_act) == Inspire_Num_Motors:
                        with self.left_hand_force_array.get_lock():
                            for i in range(Inspire_Num_Motors):
                                self.left_hand_force_array[i] = float(left_state_msg.force_act[i])
                else:
                ...
                if hasattr(right_state_msg, 'angle_act') and len(right_state_msg.angle_act) == Inspire_Num_Motors:
                    ...
                    if hasattr(right_state_msg, 'force_act') and len(right_state_msg.force_act) == Inspire_Num_Motors:
                        with self.right_hand_force_array.get_lock():
                            for i in range(Inspire_Num_Motors):
                                self.right_hand_force_array[i] = float(right_state_msg.force_act[i])
                else:
            ...
            if left_touch_msg is not None:
                flat_left_touch = self._flatten_touch_msg(left_touch_msg)
                with self.left_hand_touch_array.get_lock():
                    self.left_hand_touch_array[:] = flat_left_touch

            right_touch_msg = self.RightHandTouch_subscriber.Read()
            if right_touch_msg is not None:
                flat_right_touch = self._flatten_touch_msg(right_touch_msg)
                with self.right_hand_touch_array.get_lock():
                    self.right_hand_touch_array[:] = flat_right_touch

            time.sleep(0.002)

    def _flatten_touch_msg(self, touch_msg):
        flat_values = []
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
                              left_hand_force_array, right_hand_force_array, left_hand_touch_array, right_hand_touch_array,
                              dual_hand_data_lock = None, dual_hand_state_array = None, dual_hand_action_array = None,
                              dual_hand_force_array = None, dual_hand_touch_array = None):
        ...
        try:
            while self.running:
                ...
                # Read left and right q_state from shared arrays
                state_data = np.concatenate((np.array(left_hand_state_array[:]), np.array(right_hand_state_array[:])))
                force_data = np.concatenate((np.array(left_hand_force_array[:]), np.array(right_hand_force_array[:])))
                ...
                if dual_hand_state_array and dual_hand_action_array:
                    with dual_hand_data_lock:
                        dual_hand_state_array[:] = state_data
                        dual_hand_action_array[:] = action_data
                        if dual_hand_force_array is not None:
                            dual_hand_force_array[:] = force_data
                        if dual_hand_touch_array is not None:
