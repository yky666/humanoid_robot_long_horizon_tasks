...
class RerunEpisodeReader:
    ...
    def _process_images(self, item_data, data_type, dir_path):
        images = {}
        data_dict = item_data.get(data_type) or {}
...
class RerunLogger:
    def __init__(self, prefix = "", IdxRangeBoundary = 30, memory_limit = None, spawn_viewer = True, save_path = None):
        if rr is None or rrb is None:
            raise ImportError(
                "rerun is not installed. Install the rerun Python package before using visualization replay."
            )
        self.prefix = prefix
        ...
        if self.IdxRangeBoundary and (self.spawn_viewer or self.save_path):
    ...
    def log_item_data(self, item_data: dict):
        ...
        for part, state_info in states.items():
            if part != "body" and state_info:
                ...
                for idx, val in enumerate(values):
                    rr.log(f"{self.prefix}{part}/states/qpos/{idx}", rr.Scalars([float(val)]))
        ...
        for part, action_info in actions.items():
            if part != "body" and action_info:
                ...
                for idx, val in enumerate(values):
                    rr.log(f"{self.prefix}{part}/actions/qpos/{idx}", rr.Scalars([float(val)]))
...
if __name__ == "__main__":
    ...
    # # 把这段代码释放出来，并将 data_dir 改成你自己的目录
    # data_dir = "./data/pick cube" # 👈 注意这里改成你的目录
    # episode_data_number = 1       # 👈 改成你刚才录的 episode_0001
    # episode_reader2 = RerunEpisodeReader(task_dir = data_dir)
    # user_input = input("Please enter the start signal (enter 'on' to start the subsequent program):\n")
    # episode_data8 = episode_reader2.return_episode_data(episode_data_number)
    ...
    # 1. 明确指向你自己录制的真实数据存放位置！
    data_dir = "./utils/data/pick cube" 
    episode_data_number = 1  # 👈 对应你刚才保存的 episode_0002

    try:
        # 2. 读取你的数据
        episode_reader1 = RerunEpisodeReader(task_dir=data_dir)
        episode_data = episode_reader1.return_episode_data(episode_data_number)
        
        # 3. 启动 Rerun 引擎进行回放
        logger_mp.info(f"🚀 开始回放 {data_dir}/episode_{episode_data_number:04d} ...")
        
        # 这里用 60 帧的滑动窗口，限制内存占用
        online_logger = RerunLogger(prefix="offline/", IdxRangeBoundary=60)
        
        for item_data in episode_data:
            online_logger.log_item_data(item_data)
            time.sleep(0.033) # 模拟真实的 30Hz 采集频率
            
        logger_mp.info("✅ 离线可视化回放完毕。")
        
        # 让 Rerun 窗口保持打开，防止回放完秒退
        input("按下回车键退出...") 
        
    except Exception as e:
        logger_mp.error(f"❌ 回放失败: {e}")
