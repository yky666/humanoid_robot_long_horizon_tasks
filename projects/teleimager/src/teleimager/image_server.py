...
class RealSenseCamera(BaseCamera):
        ...
        try:
            ...
            for dev in ctx.query_devices():
                if dev.get_info(rs.camera_info.serial_number) == self._serial_number:
                    logger_mp.info(f"[RealSenseCamera] 发送物理重启指令到相机 {self._serial_number}，请等待 3 秒...")
                    dev.hardware_reset()
                    time.sleep(3) # 给它 3 秒钟断电重启，并在 USB 总线上重新挂载
                    break
            # ==========================================
    ...
    def _update_frame(self):
        ...
        try:
            ...
            # 【核心修复：把画面推给服务器，并告诉服务器准备完毕】
            if self._enable_webrtc:
                self._webrtc_buffer.write(color_image)
            ...
            logger_mp.warning(f"[RealSenseCamera] {self._cam_topic} 警告：取帧超时或丢帧 ({e})，正在自动重试...")
        except Exception as e:
            logger_mp.error(f"[RealSenseCamera] {self._cam_topic} 取帧发生未知错误: {e}")
...
###自定义DJI的相机类
class DJIV4L2Camera(BaseCamera):
    def __init__(self, cam_topic, video_id, img_shape, fps, 
                 enable_zmq=True, zmq_port=55555, enable_webrtc=False, webrtc_port=66666, webrtc_codec=None):
        super().__init__(cam_topic, img_shape, fps, enable_zmq, zmq_port, enable_webrtc, webrtc_port, webrtc_codec)
        
        logger_mp.info(f"🔥 [DJIV4L2] 启动大疆 V4L2 直连通道: /dev/video{video_id}")
        # 严格使用数字 video_id 和 V4L2，完美避开报错
        self.cap = cv2.VideoCapture(video_id, cv2.CAP_V4L2)
        ...
        if not self._can_read_frame():
            self.release()
            raise RuntimeError(f"[DJIV4L2] Camera {self._cam_topic} 读图失败！")
        else:
            logger_mp.info(str(self))

    def __str__(self):
        return f"[DJIV4L2: {self._cam_topic}] initialized."
        
    def _can_read_frame(self):
        # 🚀 就是这极其关键的 2 秒钟预热！
        logger_mp.info(f"⏳ [DJIV4L2] 等待大疆相机预热 (2秒)...")
        time.sleep(2.0)
        for _ in range(10):
            success, _ = self.cap.read()
            if success:
                logger_mp.info(f"🎉 [DJIV4L2] 成功抽出第一帧！")
                return True
            time.sleep(0.5)
        return False
    ...
    def release(self):
        if hasattr(self, 'cap') and self.cap is not None:
            self.cap.release()
            self.cap = None
        logger_mp.info(f"[DJIV4L2] Released {self._cam_topic}")
...
class OpenCVCamera(BaseCamera):
    def __init__(self, cam_topic, video_path, img_shape, fps, 
        ...
        # 🚀 [修复 1] 剥离字符串，提取纯数字索引 (解决 capture by name 警告)
        if isinstance(video_path, str) and video_path.startswith("/dev/video"):
            device_index = int(video_path.split("video")[-1])
        else:
            device_index = video_path
    ...
    def _can_read_frame(self):
        ...
        # 🚀 [大疆专属预热] 强制程序休眠 2 秒，等待相机内部芯片完成握手
        logger_mp.info(f"⏳ [OpenCVCamera] 正在等待 {self._cam_topic} 硬件预热 (约需2秒)...")
        time.sleep(2.0) 
        
        for i in range(15): # 扩大重试次数
            success, _ = self.cap.read()
            if success:
                logger_mp.info(f"🎉 [OpenCVCamera] {self._cam_topic} 画面提取成功！")
                return True
            time.sleep(0.2)
        return False
...
class ImageServer:
        ...
        try:
            ...
            for cam_topic, cam_cfg in self._cam_config.items():
                ...
                if cam_type == "opencv":
                    ...
                    # # 🚀 [自动雷达] 无视 YAML 配置，开机自动全盘扫描大疆相机！
                    # dji_target_id = None
                    # for idx in range(30):  # 扫描 0~29 号节点
                    #     sys_name_path = f"/sys/class/video4linux/video{idx}/name"
                    #     if os.path.exists(sys_name_path):
                    #         with open(sys_name_path, "r") as f:
                    #             cam_name = f.read()
                    #             # 只要名字里带 DJI 或者 Action，立刻锁定！
                    #             if "DJI" in cam_name or "Action" in cam_name:
                    #                 dji_target_id = idx
                    #                 break # DJI 通常会有两个节点，第一个就是真实视频流，直接 break
                    
                    # if dji_target_id is not None:
                    #     v_id = dji_target_id
                    #     logger_mp.info(f"🎯 自动雷达锁定大疆真实节点: /dev/video{v_id}")
                    # else:
                    #     # 万一没插大疆，或者没开 Webcam 模式，退回默认配置
                    #     v_id = int(video_id) if video_id else 0
                    #     logger_mp.warning(f"⚠️ 未扫描到大疆，退回默认节点: /dev/video{v_id}")

                    # 🚀 直接读取 yaml 里的 video_id，精准分配左右手！
                    v_id = int(video_id) if video_id else 0

                    # 🚀 召唤大疆专属 V4L2 神器！
                    self._cameras[cam_topic] = DJIV4L2Camera(cam_topic, v_id, img_shape, fps,
                                                             enable_zmq, zmq_port, enable_webrtc, webrtc_port, webrtc_codec)
        ...
        try:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    # 把 error 改为 exception，这样不仅会打印这段话，还会把完整的报错堆栈（红字）全部吐出来
                    logger_mp.exception(f"[Image Server] Error updating frame for {cam_topic} camera: {e}")
                    
                    # 如果原代码下面是 break，请把它注释掉，换成 continue！
                    # 这样即使 RealSense 偶尔卡顿超时丢了一帧，它也会继续去拿下一帧，而不是直接自杀。
                    continue
    ...
    def start(self):
        ...
        for camera_topic, camera in self._cameras.items():
            ready = camera.wait_until_ready(timeout=60.0)
            if not ready:
