...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "/mnt/data3/zhangjian/hf_cache/hub/models--JianZhangAI--trained_model_early_exit/snapshots/250746ac29c8850549a95f9565d7004f0c19a2af/libero_4_molmo-7b-09242207_clip_l1_regression_early_exit_wrist_proprio_ft_ah_fullyft_llm_bs224/step41500-unsharded"     # Pretrained checkpoint path
    ...
    rlds_data_root_dir: str = "/mnt/data/zhangjian/dataset/Simulation/datasets--openvla--modified_libero_rlds"
    ...
    # truncated 的部分，每一层只去噪很少的步数，传递给下一层
    truncated_denoise_steps: int = 2
...
def setup_logging(cfg: GenerateConfig):
    ...
    # Create run ID
    run_id = f"EVAL-{cfg.task_suite_name}-{cfg.model_family}-fm_truncated-exit_dist-{cfg.exit_dist}-exit_ratio-{cfg.exit_ratio}-{DATE_TIME}"
...
def initialize_and_load_model(generate_cfg):
    ...
    if cfg.load_path == "debug":
        ...
        assert config.model.action_head == "flow_matching", "Only flow matching action head is supported for this script"
        # 把去噪的步数设置为 truncated_denoise_steps
        config.model.num_diffusion_inference_steps = generate_cfg.truncated_denoise_steps
...
def initialize_exit_controller(generate_cfg, model,dataloader,device):
    ...
    value_net = ActionValueNet(exit_list=model.get_all_exit_idx(cfg.exit_interval), exit_head=model.action_head, model=model, 
                                interval=cfg.exit_interval, threshold_type=cfg.threshold_type, anchor=True)
    ...
    if not generate_cfg.thresholds:
        if model.config.action_head == "flow_matching":
            json_file_path = state_dict_path.with_name(f"exit_action_delta_matrix_{cfg.task_suite_name}_fm_truncated.json")
        else:
        ...
        if generate_cfg.load_threshold and values is not None: # load cached value distribution
            ...
            # 这里跑训练集校准阈值的时候，实现把noise action从上一层传递到下一层的方式
