...
def compute_vector_retarget_reference(
    retargeting,
    target_qpos: np.ndarray,
    scaling_factor: float,
) -> np.ndarray:
    optimizer = retargeting.optimizer
    robot_qpos = np.zeros(optimizer.robot.dof, dtype=np.float32)
    robot_qpos[optimizer.idx_pin2target] = target_qpos
    if optimizer.adaptor is not None:
        robot_qpos = optimizer.adaptor.forward_qpos(robot_qpos)

    optimizer.robot.compute_forward_kinematics(robot_qpos)
    target_link_poses = [
        optimizer.robot.get_link_pose(index) for index in optimizer.computed_link_indices
    ]
    body_pos = np.array([pose[:3, 3] for pose in target_link_poses])
    origin_link_pos = body_pos[optimizer.origin_link_indices.numpy(), :]
    task_link_pos = body_pos[optimizer.task_link_indices.numpy(), :]
    return (task_link_pos - origin_link_pos) / scaling_factor


class Rh56dftpThumbPostProcessor:
    THUMB_JOINT_NAMES = {
        HandType.left: ("left_thumb_1_joint", "left_thumb_swing_joint"),
        HandType.right: ("right_thumb_2_joint", "right_thumb_1_joint"),
    }

    def __init__(self, hand_type: HandType, config: RetargetingConfig):
        if config.type != "vector" or config.target_joint_names is None:
            raise ValueError("RH56DFTP thumb post-processing requires vector retargeting target joints.")

        self.thumb_joint_names = self.THUMB_JOINT_NAMES[hand_type]
        self.thumb_target_indices = np.array(
            [config.target_joint_names.index(name) for name in self.thumb_joint_names],
            dtype=int,
        )

        calibration_retargeting = config.build()
        self.thumb_qpos_indices = np.array(
            [calibration_retargeting.joint_names.index(name) for name in self.thumb_joint_names],
            dtype=int,
        )
        target_joint_limits = calibration_retargeting.joint_limits.astype(np.float32)
        self.lower_limits = target_joint_limits[self.thumb_target_indices, 0]
        self.upper_limits = target_joint_limits[self.thumb_target_indices, 1]
        self.command_spans = self.upper_limits - self.lower_limits

        basis_columns = []
        for target_index in self.thumb_target_indices:
            target_qpos = target_joint_limits[:, 0].copy()
            target_qpos[target_index] = target_joint_limits[target_index, 1]
            ref_value = compute_vector_retarget_reference(
                calibration_retargeting,
                target_qpos,
                config.scaling_factor,
            )
            calibration_retargeting.reset()
            calibration_retargeting.retarget(ref_value)
            solved_qpos = calibration_retargeting.retarget(ref_value)
            observed_thumb = solved_qpos[self.thumb_qpos_indices]
            basis_columns.append((observed_thumb - self.lower_limits)[:, None])

        self.observed_basis = np.hstack(basis_columns)
        if np.linalg.cond(self.observed_basis) > 1e3:
            raise ValueError("RH56DFTP thumb correction basis is ill-conditioned.")
        self.inverse_basis = np.linalg.inv(self.observed_basis)

    def apply(self, qpos: np.ndarray) -> np.ndarray:
        corrected_qpos = np.array(qpos, copy=True)
        observed_thumb = corrected_qpos[self.thumb_qpos_indices]
        normalized_command = self.inverse_basis @ (observed_thumb - self.lower_limits)
        normalized_command = np.clip(normalized_command, 0.0, 1.0)
        corrected_thumb = self.lower_limits + normalized_command * self.command_spans
        corrected_qpos[self.thumb_qpos_indices] = np.clip(
            corrected_thumb,
            self.lower_limits,
            self.upper_limits,
        )
        return corrected_qpos


class Rh56dftpEndpointPostProcessor:
    OPEN_FINGER_THRESHOLD = 0.08
    OPEN_THUMB_THRESHOLD = 0.08
    OPEN_THUMB_SWING_THRESHOLD = 0.10
    CLOSED_FINGER_THRESHOLD = 0.90
    CLOSED_THUMB_THRESHOLD = 0.90

    def __init__(self, config: RetargetingConfig):
        if config.target_joint_names is None:
            raise ValueError("RH56DFTP endpoint post-processing requires target_joint_names.")

        calibration_retargeting = config.build()
        self.target_qpos_indices = np.array(
            [calibration_retargeting.joint_names.index(name) for name in config.target_joint_names],
            dtype=int,
        )
        self.lower_limits = calibration_retargeting.joint_limits[:, 0].astype(np.float32)
        self.upper_limits = calibration_retargeting.joint_limits[:, 1].astype(np.float32)
        self.command_spans = self.upper_limits - self.lower_limits
        self.finger_target_indices = np.arange(4, dtype=int)
        self.thumb_pitch_target_index = 4
        self.thumb_swing_target_index = 5

    def apply(self, qpos: np.ndarray) -> np.ndarray:
        corrected_qpos = np.array(qpos, copy=True)
        target_qpos = corrected_qpos[self.target_qpos_indices]
        normalized = np.divide(
            target_qpos - self.lower_limits,
            self.command_spans,
            out=np.zeros_like(target_qpos),
            where=self.command_spans > 1e-6,
        )

        finger_closure = normalized[self.finger_target_indices]
        thumb_pitch_closure = normalized[self.thumb_pitch_target_index]
        thumb_swing_closure = normalized[self.thumb_swing_target_index]

        if (
            np.max(finger_closure) <= self.OPEN_FINGER_THRESHOLD
            and thumb_pitch_closure <= self.OPEN_THUMB_THRESHOLD
            and thumb_swing_closure <= self.OPEN_THUMB_SWING_THRESHOLD
        ):
            corrected_qpos[self.target_qpos_indices] = self.lower_limits
        elif (
            np.min(finger_closure) >= self.CLOSED_FINGER_THRESHOLD
            and thumb_pitch_closure >= self.CLOSED_THUMB_THRESHOLD
        ):
            corrected_qpos[self.target_qpos_indices] = self.upper_limits

        return corrected_qpos


class Rh56dftpHumanClosurePostProcessor:
    FINGER_STRAIGHTNESS_OPEN = 0.95
    FINGER_STRAIGHTNESS_CLOSED = 0.45
    THUMB_STRAIGHTNESS_OPEN = 0.95
    THUMB_STRAIGHTNESS_CLOSED = 0.78
    CLOSURE_BLEND_ALPHA = 0.9
    FINGER_OPEN_CLOSURE_SNAP_THRESHOLD = 0.10
    THUMB_OPEN_CLOSURE_SNAP_THRESHOLD = 0.12
    ...
    TARGET_THUMB_PITCH_JOINT_NAMES = {
        HandType.left: "left_thumb_1_joint",
        HandType.right: "right_thumb_2_joint",
    }

    def __init__(self, hand_type: HandType, config: RetargetingConfig):
        if config.target_joint_names is None:
            raise ValueError("RH56DFTP human closure post-processing requires target_joint_names.")
        ...
        self.upper_limits = calibration_retargeting.joint_limits[:, 1].astype(np.float32)
        self.command_spans = self.upper_limits - self.lower_limits
        self.finger_target_indices = np.array(
            [config.target_joint_names.index(name) for name in self.TARGET_FINGER_JOINT_NAMES[hand_type]],
            dtype=int,
        )
        self.thumb_pitch_target_index = config.target_joint_names.index(
            self.TARGET_THUMB_PITCH_JOINT_NAMES[hand_type]
        )

    @staticmethod
    def _compute_straightness(joint_pos: np.ndarray, chain: tuple[int, int, int, int]) -> float:
        points = joint_pos[np.array(chain, dtype=int)]
        segment_lengths = np.linalg.norm(np.diff(points, axis=0), axis=1)
        chain_length = float(np.sum(segment_lengths))
        if chain_length <= 1e-6:
            return 1.0
        direct_length = float(np.linalg.norm(points[-1] - points[0]))
        return direct_length / chain_length

    @staticmethod
    def _straightness_to_closure(
        straightness: float,
        open_threshold: float,
        closed_threshold: float,
    ) -> float:
        closure = (open_threshold - straightness) / max(open_threshold - closed_threshold, 1e-6)
        return float(np.clip(closure, 0.0, 1.0))

    def _compute_human_closure(self, joint_pos: np.ndarray) -> np.ndarray:
        finger_closure = []
        for finger_name in ("little", "ring", "middle", "index"):
            straightness = self._compute_straightness(joint_pos, self.FINGER_CHAINS[finger_name])
            finger_closure.append(
                self._straightness_to_closure(
                    straightness,
                    self.FINGER_STRAIGHTNESS_OPEN,
                    self.FINGER_STRAIGHTNESS_CLOSED,
                )
            )

        thumb_straightness = self._compute_straightness(joint_pos, self.FINGER_CHAINS["thumb"])
        thumb_pitch_closure = self._straightness_to_closure(
            thumb_straightness,
            self.THUMB_STRAIGHTNESS_OPEN,
            self.THUMB_STRAIGHTNESS_CLOSED,
        )

        return np.array(finger_closure + [thumb_pitch_closure], dtype=np.float32)

    def apply(self, qpos: np.ndarray, joint_pos: np.ndarray) -> np.ndarray:
        corrected_qpos = np.array(qpos, copy=True)
        target_qpos = corrected_qpos[self.target_qpos_indices]
        normalized_target_qpos = np.divide(
            target_qpos - self.lower_limits,
            self.command_spans,
            out=np.zeros_like(target_qpos),
            where=self.command_spans > 1e-6,
        ...
        human_closure = self._compute_human_closure(joint_pos)
        blended_finger_closure = (
            (1.0 - self.CLOSURE_BLEND_ALPHA) * normalized_target_qpos[self.finger_target_indices]
            + self.CLOSURE_BLEND_ALPHA * human_closure[:4]
        )
        blended_finger_closure = np.where(
            human_closure[:4] <= self.FINGER_OPEN_CLOSURE_SNAP_THRESHOLD,
            0.0,
            blended_finger_closure,
        )
        normalized_target_qpos[self.finger_target_indices] = blended_finger_closure

        blended_thumb_pitch_closure = (
            (1.0 - self.CLOSURE_BLEND_ALPHA) * normalized_target_qpos[self.thumb_pitch_target_index]
            + self.CLOSURE_BLEND_ALPHA * human_closure[4]
        )
        if human_closure[4] <= self.THUMB_OPEN_CLOSURE_SNAP_THRESHOLD:
            blended_thumb_pitch_closure = 0.0
        normalized_target_qpos[self.thumb_pitch_target_index] = blended_thumb_pitch_closure

        corrected_target_qpos = self.lower_limits + np.clip(normalized_target_qpos, 0.0, 1.0) * self.command_spans
        corrected_qpos[self.target_qpos_indices] = corrected_target_qpos
        return corrected_qpos
...
class InspireHandDDSPublisher:
    def __init__(
        ...
        try:
            ...
            raise ValueError(
                "Inspire SDK publishing is currently only supported for inspire and rh56dftp hands."
            ) from exc
...
def start_retargeting(
    ...
    rh56dftp_thumb_postprocessor = None
    rh56dftp_human_closure_postprocessor = None
    rh56dftp_endpoint_postprocessor = None
    if robot_name is RobotName.rh56dftp and config.type == "vector":
        rh56dftp_thumb_postprocessor = Rh56dftpThumbPostProcessor(
            hand_type=hand_type,
            config=config,
        )
        rh56dftp_human_closure_postprocessor = Rh56dftpHumanClosurePostProcessor(
            hand_type=hand_type,
            config=config,
        )
        rh56dftp_endpoint_postprocessor = Rh56dftpEndpointPostProcessor(config=config)
        logger.info("RH56DFTP thumb decoupling post-processing enabled")
        logger.info("RH56DFTP human closure range post-processing enabled")
        logger.info("RH56DFTP endpoint snap post-processing enabled")
    ...
    if robot_name is not RobotName.rh56dftp:
    ...
    cam = scene.add_camera(
        name="Cheese!", width=400, height=400, fovy=1, near=0.1, far=10
    )
    cam.set_local_pose(sapien.Pose([0.50, 0, 0.0], [0, 0, 0, -1]))
    if robot_name is RobotName.rh56dftp:
        cam.set_local_pose(sapien.Pose([0.36, 0, 0.0], [0, 0, 0, -1]))
    ...
    elif "rh56dftp" in robot_model_name:
        robot.set_pose(sapien.Pose([0, 0, -0.05], [0, 0, 1, 0]))
    ...
    def _render_loop():
        ...
        while not _render_stop.is_set():
            ...
            human_bgr_resized = cv2.resize(bgr_snap, (400, 400))
            combined_frame = cv2.hconcat([human_bgr_resized, render_bgr])
            _, buffer = cv2.imencode('.jpg', combined_frame, [cv2.IMWRITE_JPEG_QUALITY, 50])
    ...
    while True:
        ...
        if joint_pos is None:
            ...
            if rh56dftp_thumb_postprocessor is not None:
                qpos = rh56dftp_thumb_postprocessor.apply(qpos)
            if rh56dftp_human_closure_postprocessor is not None:
                qpos = rh56dftp_human_closure_postprocessor.apply(qpos, joint_pos)
            if rh56dftp_endpoint_postprocessor is not None:
                qpos = rh56dftp_endpoint_postprocessor.apply(qpos)
            # Re-apply mimic joint constraints after post-processing
            adaptor = retargeting.optimizer.adaptor
            if adaptor is not None:
                qpos = adaptor.forward_qpos(qpos)
...
def main(
    ...
    if publish_to_inspire_sdk and robot_name not in INSPIRE_SDK_COMMAND_JOINT_NAMES:
        raise ValueError(
            "publish_to_inspire_sdk is currently only supported with --robot-name inspire or rh56dftp."
        )
