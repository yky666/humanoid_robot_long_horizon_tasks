...
def get_point_from_mask(mask):
    # input a mask, return the center point of the mask
    non_zero_indices = np.nonzero(mask)
    if len(non_zero_indices[0]) == 0:
        center_point = None
    else:
        center_x = (np.mean(non_zero_indices[0])/mask.shape[0]).round(2)
        center_y = (np.mean(non_zero_indices[1])/mask.shape[1]).round(2)
        center_point = (center_x, center_y)
    return center_point

def get_mask_point(observation,idx):
    frame_id = 2
    ...
    mask = np.zeros_like(robot_mask).astype(np.uint8)
    ...
    points = "."
    obj_point = get_point_from_mask(mask==2)
    if obj_point is not None:
        point = " <point alt='obj'>" + str(obj_point) + "</point>."
        points += point 
    target_point = get_point_from_mask(mask==3)
    if target_point is not None:
        point = " <point alt='target'>" + str(target_point) + "</point>."
        points += point
    return points
    ...
    for file in h5py_files:
        with h5py.File(file, "r") as f:
            for timestamp in f["data"].keys():
                ...
                points = get_mask_point(f["data"][timestamp]["observation"],0)
                for i in range(images.shape[0]):
                    ...
                    Image.fromarray(front_image).save(f"front_image_{i}.png")
                    Image.fromarray(wrist_image).save(f"wrist_image_{i}.png")
                    assert False
                    dataset.add_frame(
                        {
                ...
                dataset.save_episode(task=np.array(f["data"][timestamp]["instruction"])[0].decode("utf-8")+points)
